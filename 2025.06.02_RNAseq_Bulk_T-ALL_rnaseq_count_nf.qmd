...
100 bp, paired-end, strand specific, total RNA Illumina Kit
...
TARGET="$HOME/github_repos/2025-07-18_RNAseq_Bulk_T-ALL_quant/data/fastq/WTS_run_2025.05.12"

SOURCE2="/mnt/network_drives/storage-bioinfo_labq/0001_ngs_raw_data/quintarelli_c/benini_f/T-ALL/2026-01-07_WTS"
TARGET2=$(dirname $TARGET)
...
OUTDIR="$HOME/github_repos/2025-07-18_RNAseq_Bulk_T-ALL_quant/data/rnaseq_quant"
mkdir $OUTDIR
```

```{r}
PREFIX <- "2025-06-02_RNAseq_Bulk_T-ALL"
```

# Sample Metadata

Please note that the name of the fastq files will be similar to the following: "12_3456_Dnase_BM_S78_L001_R1_001.fastq", where
"12" --> 'progressive number' of the sample
"3456" --> last 4 numbers of the 'easytrack_code_at_diagnosis'; when 'easytrack_code_at_diagnosis' is not available, last 4 numbers of the 'aieop_patient_id'
...
fqs_path <- file.path("data/fastq/2026-01-07_WTS")
out <- "data/fastq_filepaths_batch2.txt"
...
if ( ! file.exists(out)){
 fqs_dir <- dir(fqs_path, 
               ...
               pattern = "*.fastq.gz") %>%
               as.data.frame()
  write.delim(fqs_dir, out, sep="\t", col.names = FALSE, row.names = FALSE)
}

fqs_dir <- read.delim(out, header=FALSE) %>% 
        filter(!grepl("Undetermined", V1))
...
```{r}
# define outdirs
OUTDIR="/mnt/bioinformatics/nextflow_out/2025-07-18_RNAseq_Bulk_T-ALL_quant/fastq"
PROJ_DATA_DIR <- file.path(Sys.getenv("HOME"), "github_repos/2025-07-18_RNAseq_Bulk_T-ALL_quant/data/fastq/concatenated")
dir.create(PROJ_DATA_DIR, recursive = TRUE)

# concatenate Lane 1 and Lane 2 for all samples
fqs_concat <- map_dfr(.x = fqs_dir, 
                      .f = ~ t(str_split_1(.x, pattern = "/")) %>% 
    as.data.frame()) %>% 
    select(-V1) 

head(fqs_concat)
dim(fqs_concat) #236   4
```
...
# define file locations and generate a file manifest
fqs_manifest <- fqs_concat %>% 
    # rename_all(.funs = ~ unlist(fqs_concat[1,]))  %>% 
    mutate(fastq_path = fqs_dir,
           single_end = "false",
           library_chemistry = "Illumina RNA-seq with whole exome enrichment",
    ...
    mutate(sample_number = str_split_fixed(lane, pattern = "_", n = 2)[,1],
           lane = gsub(".+_(L[1-4])_.+","\\1", lane), #56_4407_L1_ds.1c8f8e51ae26431e94dc18b19b2e6b0e
           read = gsub(".+_(R[12])_.+","\\1",fastq_file), #56_4407_S5_L001_R2_001.fastq.gz
           sample_name = str_split_fixed(fastq_file, pattern = "_L", n = 2)[,1],
    ...
    mutate(cat_command = glue::glue("echo 'Processing {sample_id}_{read}'; zcat --verbose {paste(fastq_path, collapse = ' ')} | gzip --verbose > {paste0(concatenated_fastq, '.gz')}") %>%
               unique() %>%
               c(.,"")
            ) %>%
    ungroup() %>%
...
head(fqs_manifest)
dim(fqs_manifest) #236
length(unique(fqs_manifest$sample_name)) #59
 
# write.table(fqs_manifest,
#             file = glue("data/metadata/{PREFIX}_batch2_fastq_manifest.tsv"),
#             sep = "\t")
```

```{r}
fqs_manifest <- read.delim(glue("data/metadata/{PREFIX}_batch2_fastq_manifest.tsv"), sep = "\t")
...
outfile <- "inputs.txt"
cat(cmds, file = file.path("bin",outfile), sep = "\n")
```

```{bash}
hq  --debug worker deploy-ssh $HQ_HOSTFILE \
    --on-server-lost finish-running \
    --heartbeat 30s \
    --idle-timeout 12:00:00 \
    --detect-resources all

hq worker list

# submit the job to the scheduler
hq submit --each-line=bin/inputs.txt bin/hq_compute.sh

# Resubmitting canceled jobs that were hanging
hq submit --each-line=bin/inputs.txt --array=`hq job task-ids last --filter=canceled` bin/hq_compute.sh

# define md5sums for fastqs downloaded from basespace
find /mnt/bioinformatics/nextflow_out/2025-07-18_RNAseq_Bulk_T-ALL_quant/fastq_bs_temp/ -name "*fastq.gz" > data/raw_fastqs.txt
hq submit --each-line=data/raw_fastqs.txt bash -c 'md5sum $HQ_ENTRY > ${HQ_ENTRY}.md5'
hq submit --each-line=data/fastq_filepaths_batch2.txt bash -c 'md5sum $HQ_ENTRY > ${HQ_ENTRY}.md5'
```

```{bash}
OUTDIR=""
# Resubmitting jobs that failed
#hq submit --each-line=bin/inputs.txt --array=X,Y,Z bin/hq_compute.sh
```
...
SAMPLE_SHEET <- glue("data/sample_sheets/{PREFIX}_sample_sheet_batch2.csv")

cat(DESCRIPTION_HEADER, file = SAMPLE_SHEET, sep = "\n")

sample_sheet <- fqs_manifest %>% 
  select(id = sample_id,
          read,
          single_end,
...
NXF_OUTDIR="/mnt/bioinformatics/nextflow_out/2025-07-18_RNAseq_Bulk_T-ALL_quant"
PROJ_OUTDIR="$HOME/github_repos/2025-07-18_RNAseq_Bulk_T-ALL_quant/data/rnaseq_quant"
...
./main_run.sh 2025.06.02_RNAseq_Bulk_T-ALL_batch2
```
...
trace_file <- read.delim("reports/2025-06-02_RNAseq_Bulk_T-ALL_trim_subread_prepFusions_2025-09-08_trace.txt") %>% 
    ...
    mutate(job_name = gsub(":", "_", gsub("^(.+)\\s.+", "\\1", name)),
           peak_vmem_mib = fs::fs_bytes(peak_vmem) %>% 
               ...
               gsub(" MiB", "", .) %>% 
               as.numeric(.)) %>% 
    mutate(peak_vmem_mib = case_when(
        peak_vmem_mib < 4096 ~ 4096,
        peak_vmem_mib > 40000 ~ 49152,
        TRUE ~ peak_vmem_mib
    )) %>% 
...
trace_file
# dim(trace_file)
write.csv(trace_file, "data/computational_resources/rnaseq_count_nf_resources_trace.csv",
...
```{bash}
python3 bin/create_aws_reqs_json.py
```

```{bash}
# Default region name [None]: eu-south-1
# must use us-west-2 region because that is where my user exists. Probably try to create another one in 
# aws configure

for file in $(ls -1 data/computational_resources/*aws_ec2_instance_reqs.json)
do
    echo ${file/_reqs/_types}
    aws ec2 get-instance-types-from-instance-requirements --cli-input-json file://${file} > ${file/_reqs/_types}
done
```

```{r}
ecs_instances <- dir("data/computational_resources", pattern = "*aws_ec2_instance_types.json", full.names = TRUE)
names(ecs_instances) <- gsub("^.+resources\\/(.+)_aws.+","\\1",ecs_instances)

ec2_types <- purrr::map2_dfr(.x =  ecs_instances,
                .y = names(ecs_instances),
                .f = function(ec2_json, job){
                    
                jsonlite::fromJSON(ec2_json) %>% 
                        as.data.frame() %>% 
                        mutate(job_name = job)
...
# See this next for purchase options, etc
# aws ec2 get-instance-types-from-instance-requirements
# aws ec2 describe-instance-type-offerings
```

```{r}
ec2_types_selected <- ec2_types %>% 
    group_by(job_name) %>% 
    dplyr::slice(1) %>% 
    arrange(job_name) %>% 
    janitor::clean_names()

# Missing picard mark duplicates
# ec2_types_selected
```
...
```
```{r}
# bulk upload for 1 RNA-seq sample 
aws_bulk_upload <- trace_file %>% 
    # filter(! grepl("^ref",sample_id)) %>% 
    filter(grepl("TALL-1_S1", sample_id)) %>% 
    mutate(aws_region = 'eu-south-1',
            operating_system = 'Linux',
           tenancy = 'Shared Instances',
           number_of_instances = 1,
           assumed_usage = 3,
           usage_type = 'Hours/Week',
           purchasing_option = 'spot',
           storage_type = '',
           storage_amount_per_instance_gb = '',
           provisioning_iops_per_instance_applicable_for_gp3_io1_io2 = '',
           ebs_throughput_per_instance_applicable_for_gp3_mbps = '',
           snapshot_frequency = '',
           ebs_snapshot_amount_per_instance_gb_snapshot = '') %>% 
    left_join(., ec2_types_selected, by = "job_name") %>% 
    mutate(instance_type = ifelse(is.na(instance_type), "c1.xlarge", instance_type)) %>% 
    select(group = sample_id, 
           description = job_name,
           aws_region,
           operating_system,
           instance_type,
           tenancy:purchasing_option,
           storage_type:ebs_snapshot_amount_per_instance_gb_snapshot) %>% 
    arrange(description)

aws_bulk_upload
write.csv(aws_bulk_upload, "data/computational_resources/rnaseq_count_nf_resources_aws_bulk_upload_n1.csv", row.names = FALSE, quote = FALSE)
```
